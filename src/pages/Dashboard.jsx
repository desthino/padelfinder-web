...
import { Page, PageHeader, Section, Card, Badge, LevelBadge, Empty, Btn, PlayerCard, Spinner, useIsMobile } from '../components/UI';
          ...
          <Card key={s.label}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: s.small ? 14 : 18, fontWeight: 700, color: G.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.val}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '0 1rem' }}>
            ...
            {data.games.length === 0
              ...
              : data.games.map(g => (
                <div key={g.id} onClick={() => navigate(`/games/${g.id}`)} style={{ display: 'flex', gap: 12, padding: '10px 0', borderBottom: `0.5px solid #f0f0f0`, cursor: 'pointer' }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: G.green, minWidth: 50, textTransform: 'uppercase' }}>
            ...
            {data.clubs.length === 0
              ...
              : data.clubs.map(c => (
                <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: `0.5px solid #f0f0f0` }}>
                  <div style={{ width: 34, height: 34, borderRadius: 8, background: G.greenLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>🏟️</div>
                  <div style={{ minWidth: 0 }}>
        ...
        <div>
          ...
          {data.requests.length > 0 && (
            <Section title={`Demandes (${data.requests.length})`} icon="🤝">
              {data.requests.map(r => (
                <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: `0.5px solid #f0f0f0` }}>
                  ...
                  <button onClick={() => acceptFriend(r.id)} style={{ width: 28, height: 28, borderRadius: '50%', background: G.greenLight, border: 'none', color: G.green, cursor: 'pointer', fontWeight: 700, flexShrink: 0 }}>✓</button>
                  <button onClick={() => declineFriend(r.id)} style={{ width: 28, height: 28, borderRadius: '50%', background: G.redLight, border: 'none', color: G.red, cursor: 'pointer', fontWeight: 700, flexShrink: 0 }}>✕</button>
                </div>
              ))}
            </Section>
          )}
