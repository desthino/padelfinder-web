...
export function Navbar() {
  ...
  const links = [
    ...
    { path: '/search',    label: 'Joueurs' },
    { path: '/games',     label: 'Parties' },
  ...
  return (
    <nav style={{ background: '#fff', borderBottom: `0.5px solid ${G.border}`, padding: '0 1.5rem', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 200, fontFamily: G.font }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <Btn onClick={() => navigate('/games/create')} small>+ Créer une partie</Btn>
...
export function PlayerCard({ player, onClick, actions }) {
  return (
    <div onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: `0.5px solid #f0f0f0`, cursor: onClick ? 'pointer' : undefined }}>
      ...
      {player.rating > 0 && <span style={{ fontSize: 12, color: G.yellow, fontWeight: 600 }}>⭐ {Number(player.rating).toFixed(1)}</span>}
      {actions}
    </div>
  );
}
...
export function Page({ children, maxWidth = 1100 }) {
  return (
    <div style={{ minHeight: '100vh', background: G.bg, fontFamily: G.font }}>
      <Navbar />
      <div style={{ maxWidth, margin: '0 auto', padding: '1.5rem 1rem' }}>
        {children}
      </div>
    </div>
  );
...
export function PageHeader({ title, subtitle, action }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem' }}>
      <div>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: G.text, margin: 0 }}>{title}</h1>
